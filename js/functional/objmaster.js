...
const idabove99name = pokémon.filter(p => p.id > 99).map(p => p.name);
const poisonname = pokémon.filter(p => p.types.length === 1 && p.types[0] === "poison").map(p => p.name);
const firsttypeofflying = pokémon.filter(p => p.types.length === 2 && p.types[1] === "flying").map(p => p.types[0]);
const normalcount = pokémon.filter(p => p.types.length === 1 && p.types[0] === "normal").length;
console.log(idby3);
console.log(firetype);
console.log(multitype);
console.log(names);
console.log(idabove99name);
